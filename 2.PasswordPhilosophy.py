input_raw = '''1-5 k: kkkkhkkkkkkkkkk
...
for row in input:
    (policy, pw) = row.split(": ")
    (min_max,policy_char) = policy.split(" ")
    (min, max) = list(map(int, min_max.split("-")))
    char_count = pw.count(policy_char)
...
for row in input:
    ...
    (min_max,policy_char) = policy.split(" ")
    (min, max) = list(map(int, min_max.split("-")))
    if((pw[min-1] == policy_char or pw[max-1] == policy_char) and (pw[min-1] != pw[max-1])):
        valid_count += 1
print(valid_count)