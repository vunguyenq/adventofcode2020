...
input_test_raw = '''ecl:gry pid:860033327 eyr:2020 hcl:#ff1ffd
...
input_test_raw = input_test_raw.replace('\n\n','|').replace('\n',' ')
...
# PART 1
valid_fields = [{'byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid', 'cid'},{'byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'}]
valid_passport_count = 0
for passport_raw in input:
    raw_fields = passport_raw.split(' ')
    fields = []
    for raw_field in raw_fields:
        (field, val) = raw_field.split(':')
        fields.append(field)
    if set(fields) in valid_fields:
...
for passport_raw in input:
    ...
    # check if all mandatory fields present
    if(set(passport.keys()) not in valid_fields):
        continue
    
    # validate field data
    byr, iyr, eyr, hgt, hcl, ecl, pid = passport['byr'], passport['iyr'], passport['eyr'], passport['hgt'], passport['hcl'], passport['ecl'], passport['pid']
    if not (byr.isnumeric() and int(byr) >= 1920 and int(byr) <= 2002):
        continue
    if not (iyr.isnumeric() and int(iyr) >= 2010 and int(iyr) <= 2020):
        continue
    if not (eyr.isnumeric() and int(eyr) >= 2020 and int(eyr) <= 2030):
        continue

    hgt_unit, hgt_val = hgt[-2:], hgt[:-2]
    if not (hgt_val.isnumeric() and hgt_unit in ('cm','in')):
        continue
    if (hgt_unit == 'cm'):
        if not(int(hgt_val) >= 150 and int(hgt_val) <= 193):
            continue
    if (hgt_unit == 'in'):
        if not(int(hgt_val) >= 59 and int(hgt_val) <= 76):
            continue
