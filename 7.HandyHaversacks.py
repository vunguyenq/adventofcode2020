input_raw = '''dim silver bags contain 3 posh fuchsia bags.
...
input_test_raw = '''light red bags contain 1 bright white bag, 2 muted yellow bags.
...
input_test_raw = '''shiny gold bags contain 2 dark red bags.
...
for rule in input:
    ...
    outer_bag, inner_bags_raw = rule.split(' contain ')
    inner_bags = inner_bags_raw.split(', ')
    inner_bags_list = []
    for bag in inner_bags:
        bag_split = bag.split(' ', 1)
        if bag_split[0] == 'no':
            continue
        qty, inner_bag = int(bag_split[0]), bag_split[1]
        inner_bags_list.append(inner_bag)
    if (len(inner_bags_list) > 0): 
        bags_graph[outer_bag] = inner_bags_list
#print(bags_graph)

container_count = 0
for bag in bags_graph.keys():
    if bag!= 'shiny gold': # Ignore case where 'shiny gold' contains other bags, if any
        if not(find_path(bags_graph, bag, 'shiny gold') is None):
            container_count += 1
print(container_count)
# PART 2
# Count bags recursively
def count_bags(bag_dict, root_key):
    #print(root_key)
    if not(root_key in bag_dict): # inner-most bag, stop recursion
        return 0
    branch = bag_dict[root_key]
    #print(branch)
    branch_sum = 0
    for key in branch.keys():
        branch_sum += branch[key] + branch[key] * count_bags(bag_dict,key)
    return branch_sum
...
for rule in input:
    ...
    for bag in inner_bags:
        ...
        if bag_split[0] == 'no':
            continue
        qty, inner_bag = int(bag_split[0]), bag_split[1]
        inner_bags_dict[inner_bag] = qty
    if (len(inner_bags_dict.keys()) > 0): 
        bags_graph[outer_bag] = inner_bags_dict

print(count_bags(bags_graph, 'shiny gold'))
