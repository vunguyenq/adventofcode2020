...
rules = {}
for row in input:
    if len(row) == 0: # empty divider row
        continue
    if (row[0].isnumeric()): # rules
        rule_no, rule_txt = row.split(': ')
        rule_no = int(rule_no)
        if(rule_txt.startswith('"')): # matches single character
            rule = rule_txt.replace('"','')
        else: # sequence(s) of rules 
            rule = []
            for sub_rule_txt in rule_txt.split(' | '):
                sub_rule_nums = tuple(map(int,sub_rule_txt.split(' ')))
                rule.append(sub_rule_nums)
        rules[rule_no] = rule
...
# PART 1
# Validate if a msg matches a rule number
# Return whether the left-most part of message matches, and the remainder of message after matching
def validate_rule(msg, rule_no, all_rules):
    #print(msg, rule_no)
    #if(rule_no in [8,11,42,31]):
        #print (rule_no)
    rule = all_rules[rule_no]
    if (len(msg) == 0):
        return(False, msg)
    if isinstance(rule, str): # matches a single character:
        if msg[0] == rule:
            return (True, msg[1:])
        else:
            return (False, msg)#
    else: # matches other rules
        for rule_seq in all_rules[rule_no]:
            remainder = msg
            for rule_sub_no in rule_seq:
                valid, remainder = validate_rule(remainder, rule_sub_no, all_rules)
                if(not valid):
                    break
            if(valid):
...
# main
#test = 'abbbab'
#print(test, validate_rule(test, 0, rules))
match_count = 0
for msg in messages:
    valid, remainder = validate_rule(msg, 0, rules)
    if(valid and len(remainder) == 0): # completely match
        match_count += 1
print('-'*10 + 'PART 1' + '-'*10)
print('Part 1 answer: {}'.format(match_count))

# PART 2
# Observation:
# rule "8: 42 | 42 8" ~  "8: 42 | 42 42 | 42 42 42 | 42 42 42 42 | ..."
# rule "11: 42 31 | 42 11 31" ~ "11: 42 31 | 42 42 31 31 | 42 42 42 31 31 31 | 42 42 42 42 31 31 31 31 | ..."
# => unoptimized but quick solution: generates 10 loops for rule 8 and rule 11, hope that it converges to final result before 10 loop

# Validation
#test = 'aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba' #8
#test = 'bbaabbbaaaaaabbbbbababaaaaabbaaabba' # 8
#test = 'bbaaaaaabbbbbababaaaaabbaaabba' # 42
#test = 'aaabbbbbababaaaaabbaaabba' # 42
#test = 'bbbababaaaaabbaaabba' # 42
#test = 'abaaaaabbaaabba' # 42 f 31 t
#test = 'aabbaaabba' # 31
#test = 'aabba' # 31
#print(test, validate_rule(test, 11, rules))
#exit()


match_count = 0
for msg in messages:
    rules[8] = [(42,)] # reset rule 8 for each msg
    for repeat_rule8 in range (1,10):
        valid, remainder = validate_rule(msg, 0, rules)
        if(not valid):
            rules[8] = [tuple([42]*repeat_rule8)]
        else:
            break
    if(valid and len(remainder) == 0): # completely match
        match_count += 1
...
print('-'*10 + 'PART 2' + '-'*10)
print('Part 2 answer: {}'.format(match_count))