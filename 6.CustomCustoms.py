...
sum_count = 0
for group in input:
    unique_answers.append(set(''.join (group).replace (' ', '')))
    sum_count += len(unique_answers[-1])
print(sum_count)
...
for group in input:
    ...
    for ans in group.split(' '):
        group_answers.append(set(''.join(ans)))
    sum_count += len(set.intersection(*group_answers))
print(sum_count)